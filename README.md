...
- ⚡ **实时输出流**：支持像 `ping` 这样的长时间运行命令的实时回显，及历史缓冲区 (Scrollback) 下发
- ⌨️ **控制字符支持**：正确处理 `Ctrl+C` 带外中断信号、退格防误删、Tab 补全和上下方向键历史记录
- 🎨 **现代化 GUI**：具备 ANSI 过滤、256色高亮着色与终端虚拟渲染能力，沉浸式终端布局
- 📏 **窗口大小同步**：支持 PTY 伪终端的尺寸动态同步
- 📋 **剪贴板交互**：支持右键菜单或智能 `Ctrl+C` 选区复制
...
#### 💻 终端体验
- 服务端至 GUI 的实时输出流式传输与 Scrollback 历史下发机制
- 通过 `pyte` 库处理 ANSI 转义序列，支持 256 色前景色/背景色动态高亮（Dracula / Catppuccin 风格黑底霓虹主题）
- 完善的按键支持：`Enter`, `Backspace`（只读 Prompt 防删保护）, `Tab`, `Ctrl+C` (带外信号强杀/选区复制), 方向键历史记录回溯，以及 `Home`, `End`, `Delete`, `PageUp`, `PageDown`
- 客户端向服务端实时汇报终端窗口大小变更，并支持纯纵向软换行与虚拟滚动
- ✅ 沉浸式交互体验：去除独立输入框，支持直接在终端画布内盲打输入，行为完全对齐原生 Windows Terminal / iTerm2
...
| 多客户端支持与会话隔离 | 🟢 已实现 |
| 5 秒心跳检测与离线处理 | 🟢 已实现 |
| 基础异常处理 (防崩溃/断网重传/UI防御性渲染) | 🟢 已实现 |
| 实时命令输出 (ping 持续输出) | 🟢 已实现 |
| `Ctrl+C` 中断服务端命令 | 🟢 已通过 Win32 API / POSIX tcgetpgrp 彻底解决阻塞进程强杀 |
| ANSI 转义序列过滤/渲染避免乱码 | 🟢 已通过终端虚拟渲染与前背景色 tag 解析实现 |
...
| 版本 | 日期 | 备注 |
| --- | --- | --- |
| `0.2.0` | `2026-04-21` | 企业级 UI 与交互体验升级：沉浸式多巴胺主题终端、ANSI 颜色高亮、彻底修复 Windows/POSIX `Ctrl+C` 信号中断问题、添加 Scrollback 历史下发、防误删 Prompt 保护与方向键命令历史回溯、添加剪贴板上下文菜单、自动预热 `whoami` 指令。 |
